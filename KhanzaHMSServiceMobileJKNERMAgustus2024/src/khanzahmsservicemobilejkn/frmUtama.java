...
public class frmUtama extends javax.swing.JFrame {
    ...
    private void kirimTaskID(String kodebooking, String norawat) {
        try {
            // Task 3
            processTask(kodebooking, norawat, 3, getTask3Data(norawat));

            // Task 4
            processTask(kodebooking, norawat, 4, getTask4Data(norawat));

            // Task 5
            String task5Time = getTask5Data(norawat);
            if (!task5Time.isEmpty()) {
                if (processTask(kodebooking, norawat, 5, task5Time)) {
                    System.out.println("berhasil task 5");
                } else {
                    // Handle case where Task 5 fails
                    String fallbackTime = Sequel.cariIsi("select DATE_ADD(referensi_mobilejkn_bpjs_taskid_status200.taskid4,INTERVAL 4 MINUTE) from referensi_mobilejkn_bpjs_taskid_status200 where referensi_mobilejkn_bpjs_taskid_status200.no_rawat=?", norawat);
                    processTask(kodebooking, norawat, 5, fallbackTime);
                }
//                processTask(kodebooking, norawat, 5, task5Time);
            }

            // Task 6
            String task6Time = getTask6Data(norawat);
            if (!task6Time.isEmpty()) {
                boolean task6Success = processTask(kodebooking, norawat, 6, task6Time);
                if (task6Success) {
                    // Task 7 only if Task 6 was successful
                    String task7Time = getTask7Data(norawat);
                    if (!task7Time.isEmpty()) {
                        processTask(kodebooking, norawat, 7, task7Time);
                    }
                } else {
                    // Handle case where Task 6 fails
                    String fallbackTime = Sequel.cariIsi("select DATE_ADD(referensi_mobilejkn_bpjs_taskid_status200.taskid5,INTERVAL 5 MINUTE) from referensi_mobilejkn_bpjs_taskid_status200 where referensi_mobilejkn_bpjs_taskid_status200.no_rawat=?", norawat);
                    if (processTask(kodebooking, norawat, 6, fallbackTime)) {
                        // Task 7 only if Task 6 was successful
                        String task7Time = getTask7Data(norawat);
                        if (!task7Time.isEmpty()) {
                            processTask(kodebooking, norawat, 7, task7Time);
    ...
    private boolean processTask(String kodebooking, String norawat, int taskId, String time) {
        if (time.isEmpty()) {
            return false; // No time means task cannot be processed
        }
        ...
        try {
            ...
            String requestJson = String.format("{\"kodebooking\": \"%s\", \"taskid\": \"%d\", \"waktu\": \"%d\"}", kodebooking, taskId, parsedDate.getTime());
            ...
            if (nameNode.path("code").asText().equals("200")) {
                Sequel.mengedit("referensi_mobilejkn_bpjs_taskid_status200", "no_rawat=?", "taskid" + taskId + "=?", 2, new String[]{time, norawat});
                System.out.println("Task" + taskId + " No Rawat =" + norawat + " : " + nameNode.path("message").asText());
                return true; // Task was successful
            } else {
                System.out.println("Task" + taskId + " No Rawat =" + norawat + " : " + nameNode.path("message").asText());
                return false; // Task failed
            }
        } catch (Exception ex) {
            System.out.println("Notifikasi Bridging Task" + taskId + " : " + ex);
            return false; // Task failed due to exception
        }
    }
     ...
     * Logic for Task 4.
     */
    private String getTask4Data(String norawat) {
        String datajam = Sequel.cariIsi("SELECT concat(pemeriksaan_ralan.tgl_perawatan, ' ', pemeriksaan_ralan.jam_rawat) "
                + "FROM pemeriksaan_ralan WHERE pemeriksaan_ralan.nip NOT LIKE '%D%' AND pemeriksaan_ralan.no_rawat='" + norawat + "'");
        if (datajam == null || datajam.isEmpty()) {
            datajam = Sequel.cariIsi("select penilaian_awal_keperawatan_ralan.tanggal from penilaian_awal_keperawatan_ralan where penilaian_awal_keperawatan_ralan.no_rawat=?", norawat);
            if (datajam == null || datajam.isEmpty()) {
                datajam = Sequel.cariIsi("select mutasi_berkas.diterima from mutasi_berkas where mutasi_berkas.no_rawat=? and mutasi_berkas.diterima<>'0000-00-00 00:00:00'", norawat);
                if (datajam == null || datajam.isEmpty()) {
                    datajam = Sequel.cariIsi("select DATE_ADD(referensi_mobilejkn_bpjs_taskid_status200.taskid3,INTERVAL 20 MINUTE) from referensi_mobilejkn_bpjs_taskid_status200 where referensi_mobilejkn_bpjs_taskid_status200.no_rawat=?", norawat);
                }
            }
        }
        return datajam;
    ...
    private String getTask3Data(String norawat) {
        String datajam = Sequel.cariIsi("select CONCAT(reg_periksa.tgl_registrasi,' ',reg_periksa.jam_reg) from reg_periksa where reg_periksa.no_rawat=?", norawat);
     ...
     * Logic for Task 5.
     */
    private String getTask5Data(String norawat) {
        String datajam = Sequel.cariIsi("select concat(pemeriksaan_ralan.tgl_perawatan,' ',pemeriksaan_ralan.jam_rawat) "
                + "from pemeriksaan_ralan where pemeriksaan_ralan.nip LIKE '%D%' AND pemeriksaan_ralan.no_rawat=? LIMIT 1", norawat);
        if (datajam == null || datajam.isEmpty()) {
            datajam = Sequel.cariIsi("select concat(rawat_jl_dr.tgl_perawatan,' ',rawat_jl_dr.jam_rawat) "
                    + "from rawat_jl_dr where rawat_jl_dr.no_rawat=? LIMIT 1", norawat);
            if (datajam == null || datajam.isEmpty()) {
                datajam = Sequel.cariIsi("select concat(rawat_jl_pr.tgl_perawatan,' ',rawat_jl_pr.jam_rawat) "
                        + "from rawat_jl_pr where rawat_jl_pr.no_rawat=? LIMIT 1", norawat);
                if (datajam == null || datajam.isEmpty()) {
                    datajam = Sequel.cariIsi("select concat(rawat_jl_drpr.tgl_perawatan,' ',rawat_jl_drpr.jam_rawat) "
                            + "from rawat_jl_drpr where rawat_jl_drpr.no_rawat=? LIMIT 1", norawat);
                    if (datajam == null || datajam.isEmpty()) {
                        datajam = Sequel.cariIsi("select DATE_ADD(referensi_mobilejkn_bpjs_taskid_status200.taskid4,INTERVAL 4 MINUTE) "
                                + "from referensi_mobilejkn_bpjs_taskid_status200 where referensi_mobilejkn_bpjs_taskid_status200.no_rawat=?", norawat);
                    }
                }
            }
        }
     ...
     * Logic for Task 7.
     */
    private String getTask7Data(String norawat) {
        String datajam = Sequel.cariIsi("select concat(resep_obat.tgl_perawatan,' ',resep_obat.jam) "
                + "from resep_obat where resep_obat.no_rawat=? and concat(resep_obat.tgl_perawatan,' ',resep_obat.jam)<>concat(resep_obat.tgl_peresepan,' ',resep_obat.jam_peresepan)", norawat);
        if (datajam == null || datajam.isEmpty()) {
            datajam = Sequel.cariIsi("select DATE_ADD(referensi_mobilejkn_bpjs_taskid_status200.taskid6,INTERVAL 15 MINUTE) "
                    + "from referensi_mobilejkn_bpjs_taskid_status200 where referensi_mobilejkn_bpjs_taskid_status200.no_rawat=?", norawat);
        }
        return datajam;
    }

    private String getTask6Data(String norawat) {
        String datajam = "";
        datajam = Sequel.cariIsi("select concat(resep_obat.tgl_peresepan,' ',resep_obat.jam_peresepan) from resep_obat where resep_obat.status='ralan' and resep_obat.no_rawat=?", norawat);
        return datajam;
    }

    private void kirimTaskIDMultiple(String kodebooking) {

        String norawat = "";

        // Check if kodebooking exists in the database
        ...
        kirimTaskID(kodebooking, norawat);
