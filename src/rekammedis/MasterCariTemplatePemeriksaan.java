            ...
            if(tbDokter.getSelectedRow()>-1){
                ...
                if(Sequel.menyimpantf2("pemeriksaan_ralan","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","Data",21,new String[]{
                    ...
                    if((tabModeObatUmum.getRowCount()>0)||(tabModeObatRacikan.getRowCount()>0)){
                        nomor=Valid.autoNomer3("select ifnull(MAX(CONVERT(SUBSTRING(resep_obat.no_resep,9),signed)),0) from resep_obat where resep_obat.no_resep like '"+tanggaldilakukan.replaceAll("-","")+"%'",tanggaldilakukan.replaceAll("-",""),4);    
