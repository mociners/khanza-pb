...
public final class RMPenandaanAreaOperasi extends javax.swing.JDialog {
    ...
    private void MnCetakActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnCetakActionPerformed
        if (tbObat.getSelectedRow() > -1) {
            ...
            /*
            "No.Rawat","No.RM","Nama Pasien","Tgl.Lahir","J.K.","Kode Dokter","Nama Dokter","Tanggal","Anamnesis",
            "Hubungan","Ruangan","Operasi","Kode Petugas","Nama Petugas","Keterangan","No Penandaan"jas
             */
            ...
            try {
                ...
                param.put("ttd", "http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/imagefreehand/PenandaanOperasi/ttdpasien/ttd" + TNoRw.getText().replaceAll("/", "") + ".png");
            } catch (Exception e) {
...
//            }
    ...
    private void btnAmbilActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAmbilActionPerformed
        if (TNoRw.getText().trim().equals("")) {
            ...
            ttd.setNoRm(TNoRw.getText(), TNoRM.getText(), TPasien.getText(), "imagefreehand/PenandaanOperasi/ttdpasien/", "ttd" + TNoRw.getText().replaceAll("/", "") + ".png");
            ...
            try {
                ...
                while (rs.next()) {
                    tabMode.addRow(new String[]{
                        ...
                        rs.getString("anamnesis"), rs.getString("hubungan"), rs.getString("ruangan"), rs.getString("operasi"), rs.getString("kd_petugas"), rs.getString("nama"), rs.getString("keterangan"), rs.getString("no_penandaan")
                    });
                }
            } catch (Exception e) {
    ...
    public void emptTeks() {
        ...
        Operasi.setText("");
        Keterangan.setText("");
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(penandaan_area_operasi.no_penandaan,3),signed)),0) from penandaan_area_operasi where penandaan_area_operasi.tanggal='" + Valid.SetTgl(TglAsuhan.getSelectedItem() + "") + "' ",
                "PAO" + TglAsuhan.getSelectedItem().toString().substring(6, 10) + TglAsuhan.getSelectedItem().toString().substring(3, 5) + TglAsuhan.getSelectedItem().toString().substring(0, 2), 3, No);
    ...
    private void ganti() {
        if (Sequel.mengedittf("penandaan_area_operasi", "no_rawat=?", "no_rawat=?,tanggal=?,kd_dokter=?,anamnesis=?,hubungan=?,ruangan=?,operasi=?,kd_petugas=?,keterangan=?", 10, new String[]{
            TNoRw.getText(), Valid.SetTgl(TglAsuhan.getSelectedItem() + "") + " " + TglAsuhan.getSelectedItem().toString().substring(11, 19), KdDokter.getText(), Anamnesis.getSelectedItem().toString(), Hubungan.getText(),
            Ruangan.getText(), Operasi.getText(), KdPetugas.getText(), Keterangan.getText(), tbObat.getValueAt(tbObat.getSelectedRow(), 0).toString()
        }) == true) {
            tampil();
            emptTeks();
            TabRawat.setSelectedIndex(1);
