    ...
    private void tampilPerawatan() {
        try {
            ...
            htmlContent.append(
                "<html>" +
                "<head>" +
                "<link href=\"" + getClass().getResource("/css/default.css") + "\" rel=\"stylesheet\" type=\"text/css\" />" +
                "</head>" +
                "<body>"
            );
            ...
            try {
                ...
                if (rs.next()) {
                    htmlContent.append(
                        ...
                        "<tr align='center'>" +
                        "<td valign='top' width='4%' bgcolor='red'><font color='white'><b>No.</b></font></td>" +
                        "<td valign='top' width='96%' bgcolor='red'><font color='white'><b>Berkas Digital</b></font></td>" +
                        "</tr>"
                    );
                    rs.beforeFirst();
                    int w = 1;
                    while (rs.next()) {
                        String lokasi = rs.getString("lokasi_file").toLowerCase();
                        if (lokasi.contains(".jpg") || lokasi.contains(".jpeg") || lokasi.contains(".png")) {
                            htmlContent.append(
                                "<tr>" +
                                "<td valign='top' align='center'>" + w + "</td>" +
                                "<td valign='top' align='center'>" + rs.getString("nama") + "<br>" +
                                "<a href='http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/berkasrawat/" + rs.getString("lokasi_file") + "'>" +
                                "<img alt='Berkas Digital' src='http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/berkasrawat/" + rs.getString("lokasi_file") + "' width='450' height='450'/>" +
                                "</a></td>" +
                                "</tr>"
                            );
                        } else {
                            htmlContent.append(
                                ...
                                "<td valign='top'>" +
                                "<a href='http://" + koneksiDB.HOSTHYBRIDWEB() + ":" + koneksiDB.PORTWEB() + "/" + koneksiDB.HYBRIDWEB() + "/berkasrawat/" + rs.getString("lokasi_file") + "'>" + 
                                rs.getString("nama") + "_" + rs.getString("lokasi_file").replaceAll("pages/upload/", "") +
                                "</a></td>" +
                                "</tr>"
                            );
                        }
                    ...
                    htmlContent.append("<div align='center'>Data Berkas Digital Tidak Ditemukan / Belum Ada</div>");
                }
            } finally {
                if (rs != null) rs.close();
                if (ps != null) ps.close();
            ...
            LoadHTMLRiwayatPerawatan.setText(htmlContent.toString());
        } catch (Exception e) {
