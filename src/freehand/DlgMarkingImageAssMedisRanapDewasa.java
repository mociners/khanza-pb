 ...
 * @author perpustakaan
 */
public class DlgMarkingImageAssMedisRanapDewasa extends javax.swing.JDialog {
     ...
     * @param parent
     * @param modal */
    public DlgMarkingImageAssMedisRanapDewasa(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
...
//        setSize(885,674); 
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        try {
            File file = new File("tmpImageFreehand/assMdsRanapDws" + TNoRawat.getText().replaceAll("/", "") + ".png");
            if (!file.getParentFile().exists()) {
                file.getParentFile().mkdirs();
            }
            ImageIO.write(img, "png", file);
        } catch (IOException ex) {
            Logger.getLogger(DlgMarkingImageAssMedisRanapDewasa.class.getName()).log(Level.SEVERE, null, ex);
        }

        uploadImage("assMdsRanapDws" + TNoRawat.getText().replaceAll("/", "") + ".png", "assesmentmedisigd/imagemarking");

        if (Sequel.cariInteger("select count(no_rawat) as jumlah from asesmen_medis_ranap_dewasa_image_marking where no_rawat='" + TNoRawat.getText() + "'") > 0) {
            Sequel.mengedittf("asesmen_medis_ranap_dewasa_image_marking", "no_rawat=?", "tanggal=?,jam=?,url_image=?", 4, new String[]{
                tanggalNow.format(new Date()),
                jamNow.format(new Date()),
                "assesmentmedisigd/imagemarking/assMdsRanapDws" + TNoRawat.getText().replaceAll("/", "") + ".png",
                TNoRawat.getText()
            });
        } else {
            Sequel.menyimpantf("asesmen_medis_ranap_dewasa_image_marking", "?,?,?,?", "No.Rawat", 4, new String[]{
                TNoRawat.getText(),
                tanggalNow.format(new Date()),
                jamNow.format(new Date()),
                "assesmentmedisigd/imagemarking/assMdsRanapDws" + TNoRawat.getText().replaceAll("/", "") + ".png"
    ...
    */
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            DlgMarkingImageAssMedisRanapDewasa dialog = new DlgMarkingImageAssMedisRanapDewasa(new javax.swing.JFrame(), true);
...
//        Sequel.cariIsi("select nm_pasien from pasien where no_rkm_medis=? ",TPasien,TNoRM.getText());
    ...
    public void setNoRw(String norw) {
       TNoRawat.setText(norw);
        urlImage = Sequel.cariIsi("select url_image from asesmen_medis_ranap_dewasa_image_marking where no_rawat='" + norw + "' ");
