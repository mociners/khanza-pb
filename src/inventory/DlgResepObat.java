...
public final class DlgResepObat extends javax.swing.JDialog {
    ...
    private String now = dateFormat.format(date), lembarobat = "", status = "", rincianobat = "", finger = "",
            finger2 = "", kamar = "", namakamar = "", FileName = "", kodeberkas = "";
    ...
    private void initComponents() {
        ...
        internalFrame4.setBorder(javax.swing.BorderFactory.createTitledBorder(
                ...
                "::[ Ubah Aturan Pakai Obat Umum ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
        ...
        internalFrame5.setBorder(javax.swing.BorderFactory.createTitledBorder(
                javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)),
                "::[ Ubah Aturan Pakai Obat Racik ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION,
                javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11),
                new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(
                javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Resep Obat ]::",
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnSimpanActionPerformed
        if (TNoRw.getText().trim().equals("") || TPasien.getText().trim().equals("")) {
            ...
            if (Sequel.menyimpantf("resep_obat", "?,?,?,?,?,?,?,?,?,?", "Nomer Resep", 10, new String[] {
                ...
                JOptionPane.showMessageDialog(rootPane, "Resep Berhasil Disimpan!");
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            for (i = 0; i < tabMode.getRowCount(); i++) {
                Sequel.menyimpan("temporary_resep",
                        "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", 38,
                        new String[] {
                                "" + i, tabMode.getValueAt(i, 0).toString(), tabMode.getValueAt(i, 1).toString(),
            ...
            Valid.MyReportqry("rptResep.jasper", "report", "::[ Daftar Pemberian Obat Resep ]::",
    ...
    private void ppLembarObatActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_ppLembarObatActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            param.put("jam",
                    cmbJam.getSelectedItem() + ":" + cmbMnt.getSelectedItem() + ":" + cmbDtk.getSelectedItem());
            param.put("jk", Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?", Sequel.cariIsi(
                    "select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText())));
            param.put("alergi",
                    Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?",
                            Sequel.cariIsi("select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?",
                                    TNoRw.getText())));
            param.put("bb", Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?", Sequel.cariIsi(
                    "select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText())));
            param.put("tb", Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?", Sequel.cariIsi(
                    "select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText())));
            param.put("poli", Sequel.cariIsi("select penjab.png_jawab from penjab where penjab.kd_pj=?", Sequel.cariIsi(
                    "select reg_periksa.kd_pj from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText())));
            param.put("logo", Sequel.cariGambar("select setting.logo from setting"));
    ...
    private void BtnSimpan3ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_BtnSimpan3ActionPerformed
        if (NoResepUbah.getText().trim().equals("") || (tbTambahan.getRowCount() <= 0)) {
            ...
            for (i = 0; i < tbTambahan.getRowCount(); i++) {
                Sequel.queryu2(
                        "delete from aturan_pakai where tgl_perawatan=? and jam=? and no_rawat=? and kode_brng=?", 4,
                        new String[] {
            ...
            for (i = 0; i < tbTambahan1.getRowCount(); i++) {
                if (!tbTambahan1.getValueAt(i, 5).toString().equals("")) {
                    Sequel.queryu2(
                            ...
                            5, new String[] {
                                    tbTambahan1.getValueAt(i, 5).toString(), tbTambahan1.getValueAt(i, 0).toString(),
    ...
    private void ppLembarObat1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_ppLembarObat1ActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            try {
                i = 0;
                // BAGIAN 1: QUERY UNTUK OBAT NON-RACIKAN (DENGAN SUBQUERY)
                ps = koneksi.prepareStatement(
                        "SELECT " +
                                "    detail_pemberian_obat.jml, " +
                                "    IFNULL(kds.satuan, '') AS satuan, " +
                                "    db.nama_brng, " +
                                "    (SELECT IFNULL(ap.aturan, '') " +
                                "     FROM aturan_pakai ap " +
                                "     WHERE ap.no_rawat = detail_pemberian_obat.no_rawat " +
                                "       AND ap.tgl_perawatan = detail_pemberian_obat.tgl_perawatan " +
                                "       AND ap.jam = detail_pemberian_obat.jam " +
                                "       AND ap.kode_brng = detail_pemberian_obat.kode_brng " +
                                "    ) AS aturan " +
                                "FROM detail_pemberian_obat " +
                                "INNER JOIN databarang db ON detail_pemberian_obat.kode_brng = db.kode_brng " +
                                "INNER JOIN resep_obat ON detail_pemberian_obat.no_rawat = resep_obat.no_rawat AND detail_pemberian_obat.tgl_perawatan = resep_obat.tgl_perawatan AND detail_pemberian_obat.jam = resep_obat.jam "
                                +
                                "LEFT JOIN kodesatuan kds ON db.kode_sat = kds.kode_sat " +
                                "WHERE resep_obat.no_resep = ? " +
                                "AND detail_pemberian_obat.kode_brng NOT IN (SELECT kode_brng FROM detail_obat_racikan WHERE tgl_perawatan=resep_obat.tgl_perawatan AND jam=resep_obat.jam AND no_rawat=resep_obat.no_rawat)");
                try {
                    ps.setString(1, NoResep.getText());
                    rs = ps.executeQuery();
                    while (rs.next()) {
                        String namaObat = rs.getString("nama_brng");
                        String aturanPakai = rs.getString("aturan").trim(); // Membersihkan spasi
                        Sequel.menyimpan("temporary_resep",
                                "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", 38,
                                new String[] {
                                        "" + i, namaObat, aturanPakai, rs.getString("jml"), rs.getString("satuan"), "",
                                        "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",
                                        "", "", "", "", "", "", "", "", "", "", "", akses.getalamatip()
                                });
                        i++;
                    ...
                    System.out.println("Notif 1 (Obat Non-Racikan) : " + e);
                } finally {
                    if (rs != null) {
                        rs.close();
                    }
                ...
                // BAGIAN 2: QUERY UNTUK OBAT RACIKAN (DENGAN LEFT JOIN)
                psracikan = koneksi.prepareStatement(
                        "SELECT " +
                                "    o.no_racik, o.nama_racik, o.tgl_perawatan, o.jam, o.no_rawat, " +
                                "    IFNULL(o.aturan_pakai, '') AS aturan_pakai, " +
                                "    o.jml_dr, " +
                                "    IFNULL(m.nm_racik, '') AS nm_racik " +
                                "FROM obat_racikan o " +
                                "INNER JOIN resep_obat ro ON o.no_rawat = ro.no_rawat AND o.tgl_perawatan = ro.tgl_perawatan AND o.jam = ro.jam "
                                +
                                "LEFT JOIN metode_racik m ON o.kd_racik = m.kd_racik " +
                                "WHERE ro.no_resep = ?");
                try {
                    ...
                    while (rsracikan.next()) {
                        ...
                        String aturanPakaiRacikan = rsracikan.getString("aturan_pakai").trim(); // Membersihkan spasi

                        Sequel.menyimpan("temporary_resep",
                                "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", 38,
                                new String[] {
                                        "" + i, rsracikan.getString("nama_racik") + " (" + rincianobat + ")",
                                        aturanPakaiRacikan, rsracikan.getString("jml_dr"),
                    ...
                    System.out.println("Notif 2 (Obat Racikan) : " + e);
                } finally {
                    if (rsracikan != null) {
                        rsracikan.close();
                    }
            ...
            // BAGIAN 3: PARAMETER DAN PEMANGGILAN REPORT (TIDAK BERUBAH)
            Map<String, Object> param = new HashMap<>();
            param.put("namars", akses.getnamars());
            param.put("alamatrs", akses.getalamatrs());
            param.put("kotars", akses.getkabupatenrs());
            ...
            param.put("jam",
                    cmbJam.getSelectedItem() + ":" + cmbMnt.getSelectedItem() + ":" + cmbDtk.getSelectedItem());
            param.put("logo", Sequel.cariGambar("select setting.logo from setting"));
            kamar = Sequel.cariIsi("select ifnull(kd_kamar,'') from kamar_inap where no_rawat='" + TNoRw.getText()
            ...
            param.put("lahir", Sequel.cariIsi(
                    "select DATE_FORMAT(pasien.tgl_lahir,'%d-%m-%Y')as tgl_lahir from pasien where pasien.no_rkm_medis =?",
                    TNoRm.getText()));
            param.put("umur",
                    Sequel.cariIsi("select concat(umurdaftar,' ',sttsumur)as umur from reg_periksa where no_rawat =?",
                            TNoRw.getText()));
            param.put("alergi",
    ...
    private void ppLabelDataObatActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_ppLabelDataObatActionPerformed
        if (tabMode.getRowCount() == 0) {
                        ...
                        Sequel.menyimpan("temporary_resep",
                                ...
                                new String[] {
                                        ...
                                        Sequel.cariIsi("select aturan from aturan_pakai where tgl_perawatan='"
                                                + rs.getString("tgl_perawatan") + "' and jam='" + rs.getString("jam")
                                                + "' and no_rawat='" + rs.getString("no_rawat") + "' and kode_brng='"
                                                + rs.getString("kode_brng") + "'"),
                                        rs.getString("jml"), Valid.SetAngka(rs.getDouble("total")), "", "", "", "", "",
                                        "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",
                                        "", "", "", "", "", "", "", akses.getalamatip()
                                });
                        i++;
            ...
            param.put("noresep", NoResep.getText());
            param.put("poli",
                    Sequel.cariIsi("select poliklinik.nm_poli from poliklinik where poliklinik.kd_poli=?",
            ...
            Valid.MyReportqry("rptLabelDaftarObat.jasper", "report", "::[ Label Daftar Obat ]::",
                    "select * from temporary_resep where temporary_resep.temp37='" + akses.getalamatip()
                            + "' order by temporary_resep.no",
                    param);
            this.setCursor(Cursor.getDefaultCursor());
    ...
    private void ppLembarObat2ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_ppLembarObat2ActionPerformed
        if (tabMode.getRowCount() == 0) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        Sequel.menyimpan("temporary_resep",
                                "?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?", 38,
                                new String[] {
                                        "" + i, rs.getString("nama_brng"), rs.getString("aturan"), rs.getString("jml"),
                                        rs.getString("satuan"), "", "", "", "", "", "", "", "", "", "", "", "", "", "",
                                        "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "",
                                        akses.getalamatip()
                                });
            ...
            if (akses.getkode().equals("Admin Utama")) {
                ...
                param.put("diserahkanoleh",
                        Sequel.cariIsi("select petugas.petugas from petugas where petugas.nip=?", akses.getkode()));
            }
    ...
    private void ppResepTelaahActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_ppResepTelaahActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            param.put("diagnosa", Sequel.cariIsi(
                    ...
                    TNoRw.getText()));
            param.put("resep_identifikasi_pasien", Sequel.cariIsi(
                    "select resep_identifikasi_pasien from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_ket_identifikasi_pasien", Sequel.cariIsi(
                    "select resep_ket_identifikasi_pasien from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_tepat_obat",
                    Sequel.cariIsi("select resep_tepat_obat from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_ket_tepat_obat", Sequel
                    .cariIsi("select resep_ket_tepat_obat from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_tepat_dosis", Sequel
                    .cariIsi("select resep_tepat_dosis from telaah_farmasi where 	no_resep=?", NoResep.getText()));
            param.put("resep_ket_tepat_dosis", Sequel.cariIsi(
                    "select resep_ket_tepat_dosis from telaah_farmasi where 	no_resep=?", NoResep.getText()));
            param.put("resep_tepat_cara_pemberian", Sequel.cariIsi(
                    "select resep_tepat_cara_pemberian from telaah_farmasi where 	no_resep=?", NoResep.getText()));
            param.put("resep_ket_tepat_cara_pemberian", Sequel.cariIsi(
                    "select resep_ket_tepat_cara_pemberian from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_tepat_waktu_pemberian", Sequel.cariIsi(
                    "select resep_tepat_waktu_pemberian from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_ket_tepat_waktu_pemberian", Sequel.cariIsi(
                    "select resep_ket_tepat_waktu_pemberian from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_ada_tidak_duplikasi_obat", Sequel.cariIsi(
                    "select resep_ada_tidak_duplikasi_obat from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_ket_ada_tidak_duplikasi_obat",
                    Sequel.cariIsi("select resep_ket_ada_tidak_duplikasi_obat from telaah_farmasi where no_resep=?",
                            NoResep.getText()));
            param.put("resep_interaksi_obat", Sequel
                    .cariIsi("select resep_interaksi_obat from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_ket_interaksi_obat", Sequel.cariIsi(
                    "select resep_ket_interaksi_obat from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_kontra_indikasi_obat", Sequel.cariIsi(
                    "select resep_kontra_indikasi_obat from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("resep_ket_kontra_indikasi_obat", Sequel.cariIsi(
                    "select resep_ket_kontra_indikasi_obat from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("obat_tepat_pasien",
                    Sequel.cariIsi("select obat_tepat_pasien from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("obat_tepat_obat",
                    Sequel.cariIsi("select obat_tepat_obat from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("obat_tepat_dosis",
                    Sequel.cariIsi("select obat_tepat_dosis from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("obat_tepat_cara_pemberian", Sequel.cariIsi(
                    "select obat_tepat_cara_pemberian from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("obat_tepat_waktu_pemberian", Sequel.cariIsi(
                    "select obat_tepat_waktu_pemberian from telaah_farmasi where no_resep=?", NoResep.getText()));
            kamar = Sequel.cariIsi("select ifnull(kd_kamar,'') from kamar_inap where no_rawat='" + TNoRw.getText()
                    + "' order by tgl_masuk desc limit 1");
            if (!kamar.equals("")) {
                namakamar = kamar + ", " + Sequel.cariIsi(
            ...
            param.put("kamar", kamar);
            param.put("namakamar", namakamar);
            param.put("tanggallahir",
                    Sequel.cariIsi(
                            "select DATE_FORMAT(tgl_lahir,'%d-%m-%Y')as tgl_lahir from pasien where no_rkm_medis=?",
                            TNoRm.getText()));
            param.put("umur", Sequel.cariIsi("select umur from pasien where no_rkm_medis=?", TNoRm.getText()));
            finger2 = Sequel.cariIsi(
                    "select sha1(sidikjari.sidikjari) from sidikjari inner join pegawai on pegawai.id=sidikjari.id where pegawai.nik=?",
                    Sequel.cariIsi("select nip from telaah_farmasi where no_resep=?", NoResep.getText()));
            param.put("finger2",
                    "Dikeluarkan di " + akses.getnamars() + ", Kabupaten/Kota " + akses.getkabupatenrs()
                            + "\nDitandatangani secara elektronik oleh " + NmDokter.getText() + "\nID "
                            + (finger2.equals("") ? KdDokter.getText() : finger) + "\n" + DTPBeri.getSelectedItem());
            param.put("alergi",
                    Sequel.cariIsi("select alergi from pemeriksaan_ralan where no_rawat=?", TNoRw.getText()));
            ...
            Valid.MyReportqry("rptResepTelaah.jasper", "report", "::[ Lembar Pemberian Obat ]::",
    ...
    private void ppObatDiLuarBpjsActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_ppObatDiLuarBpjsActionPerformed
        if (tabMode.getRowCount() == 0) {
            ...
            param.put("alamat", Sequel.cariIsi(
                    "select reg_periksa.almt_pj from reg_periksa where reg_periksa.no_rawat=?", TNoRw.getText()));
            param.put("nopeserta", Sequel.cariIsi("select pasien.no_peserta from pasien where pasien.no_rkm_medis=?",
                    TNoRm.getText()));

            Valid.MyReportqry("rptLembarKeteranganBpjs.jasper", "report", "::[ Lembar Pemberian Obat ]::",
                    "select * from temporary_resep where temporary_resep.temp37='" + akses.getalamatip()
                            + "' order by temporary_resep.no",
                    param);
            this.setCursor(Cursor.getDefaultCursor());
    ...
    private void ppUploadPDFActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_ppUploadPDFActionPerformed
        FileName = "RESEP_" + tbResep.getValueAt(tbResep.getSelectedRow(), 0).toString().trim() + "_"
                + tbResep.getValueAt(tbResep.getSelectedRow(), 2).toString().replaceAll("[/, ]", "_");
        CreatePDF(FileName);
                        ...
                        while (rs2.next()) {
                            tabMode.addRow(new String[] {
                                    ...
                                    Sequel.cariIsi("select aturan from aturan_pakai where tgl_perawatan='"
                                            ...
                                            "jam='" + rs.getString("jam") + "' and no_rawat='"
            ...
            try {
                ...
                while (rs2.next()) {
                    tabmodeUbahRacikan.addRow(new String[] {
                            ...
                            Sequel.cariIsi("select aturan from aturan_pakai where tgl_perawatan='"
                                    + rs2.getString("tgl_perawatan") + "' and " +
                                    "jam='" + rs2.getString("jam") + "' and no_rawat='" + rs2.getString("no_rawat")
                                    + "' and kode_brng='" + rs2.getString("kode_brng") + "'")
                    });
                }
            } catch (Exception e) {
                System.out.println("Notif : " + e);
                    ...
                    if (rs.next()) {
                        LoadHTML2.setText(
                                "<html>" +
                                        ...
                                        + rs.getString("resep_identifikasi_pasien") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'></td>" +
                                        "<td valign='middle' align='left' colspan='2'>Keterangan : "
                                        + rs.getString("resep_ket_identifikasi_pasien") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>2</td>" +
                                        "<td valign='middle' align='left'>Tepat Obat</td>" +
                                        "<td valign='middle' align='center'>" + rs.getString("resep_tepat_obat")
                                        + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'></td>" +
                                        "<td valign='middle' align='left' colspan='2'>Keterangan : "
                                        + rs.getString("resep_ket_tepat_obat") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>3</td>" +
                                        "<td valign='middle' align='left'>Tepat Dosis</td>" +
                                        "<td valign='middle' align='center'>" + rs.getString("resep_tepat_dosis")
                                        + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'></td>" +
                                        "<td valign='middle' align='left' colspan='2'>Keterangan : "
                                        + rs.getString("resep_ket_tepat_dosis") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>4</td>" +
                                        "<td valign='middle' align='left'>Tepat Cara Pemberian</td>" +
                                        "<td valign='middle' align='center'>"
                                        + rs.getString("resep_tepat_cara_pemberian") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'></td>" +
                                        "<td valign='middle' align='left' colspan='2'>Keterangan : "
                                        + rs.getString("resep_ket_tepat_cara_pemberian") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>5</td>" +
                                        "<td valign='middle' align='left'>Tepat Waktu Pemberian</td>" +
                                        "<td valign='middle' align='center'>"
                                        + rs.getString("resep_tepat_waktu_pemberian") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'></td>" +
                                        "<td valign='middle' align='left' colspan='2'>Keterangan : "
                                        + rs.getString("resep_ket_tepat_waktu_pemberian") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>6</td>" +
                                        "<td valign='middle' align='left'>Ada Tidak Duplikasi Obat</td>" +
                                        "<td valign='middle' align='center'>"
                                        + rs.getString("resep_ada_tidak_duplikasi_obat") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'></td>" +
                                        "<td valign='middle' align='left' colspan='2'>Keterangan : "
                                        + rs.getString("resep_ket_ada_tidak_duplikasi_obat") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>7</td>" +
                                        "<td valign='middle' align='left'>Interaksi Obat</td>" +
                                        "<td valign='middle' align='center'>" + rs.getString("resep_interaksi_obat")
                                        + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'></td>" +
                                        "<td valign='middle' align='left' colspan='2'>Keterangan : "
                                        + rs.getString("resep_ket_interaksi_obat") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>8</td>" +
                                        "<td valign='middle' align='left'>Kontra Indikasi Obat</td>" +
                                        "<td valign='middle' align='center'>"
                                        + rs.getString("resep_kontra_indikasi_obat") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'></td>" +
                                        "<td valign='middle' align='left' colspan='2'>Keterangan : "
                                        + rs.getString("resep_ket_kontra_indikasi_obat") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        ...
                                        "<td valign='middle' align='center'>" + rs.getString("obat_tepat_pasien")
                                        + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>2</td>" +
                                        "<td valign='middle' align='left'>Tepat Obat</td>" +
                                        "<td valign='middle' align='center'>" + rs.getString("obat_tepat_obat")
                                        + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>3</td>" +
                                        "<td valign='middle' align='left'>Tepat Dosis</td>" +
                                        "<td valign='middle' align='center'>" + rs.getString("obat_tepat_dosis")
                                        + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>4</td>" +
                                        "<td valign='middle' align='left'>Tepat Cara Pemberian</td>" +
                                        "<td valign='middle' align='center'>"
                                        + rs.getString("obat_tepat_cara_pemberian") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
                                        "<td valign='middle' align='center'>5</td>" +
                                        "<td valign='middle' align='left'>Tepat Waktu Pemberian</td>" +
                                        "<td valign='middle' align='center'>"
                                        + rs.getString("obat_tepat_waktu_pemberian") + "</td>" +
                                        "</tr>" +
                                        "<tr class='isi'>" +
    ...
    private void CreatePDF(String FileName) {
        if (tabMode.getRowCount() == 0) {
            ...
            param.put("peresep", NmDokter.getText());
            param.put("noka",
                    Sequel.cariIsi("select no_peserta from pasien where pasien.no_rkm_medis=?", TNoRm.getText()));
            param.put("sep",
                    Sequel.cariIsi("select no_sep from bridging_sep where bridging_sep.no_rawat=?", TNoRw.getText()));
            ...
            param.put("poli",
                    Sequel.cariIsi("select poliklinik.nm_poli from poliklinik where poliklinik.kd_poli=?",
                            Sequel.cariIsi("select reg_periksa.kd_poli from reg_periksa where reg_periksa.no_rawat=?",
                                    TNoRw.getText())));
            String perujuk = Sequel.cariIsi("select nmppkrujukan from bridging_sep where bridging_sep.no_rawat=?",
                    TNoRw.getText());
            ...
            String diagawal = Sequel.cariIsi("select diagawal from bridging_sep where bridging_sep.no_rawat=?",
                    TNoRw.getText());
            if (diagawal == null || diagawal.isEmpty()) {
                diagawal = Sequel.cariIsi(
            ...
            Valid.MyReportPDFqryUpload("rptLembarObat3.jasper", "report", "::[ Lembar Pemberian Obat ]::",
                    "select * from temporary_resep where temporary_resep.temp37='" + akses.getalamatip()
                            + "' order by temporary_resep.no",
                    FileName, param);
            ...
            if (Sequel.cariInteger(
                ...
                uploadSuccess = Sequel.mengedittf("berkas_digital_perawatan", "lokasi_file=?",
                        "no_rawat=?,kode=?, lokasi_file=?", 4, new String[] {
                                // tbResep.getValueAt(tbResep.getSelectedRow(), 1).toString().trim(),
                                // kodeberkas, "pages/upload/" + FileName + ".pdf", "pages/upload/" + FileName +
                                // ".pdf"
                                tbResep.getValueAt(tbResep.getSelectedRow(), 2).toString().trim().substring(0,
                                        Math.min(17,
                                                tbResep.getValueAt(tbResep.getSelectedRow(), 2).toString().trim()
                                                        .length())),
                                kodeberkas, "pages/upload/" + FileName + ".pdf"
                        });
            } else {
                uploadSuccess = Sequel.menyimpantf("berkas_digital_perawatan", "?,?,?", "No.Rawat", 3, new String[] {
                        ...
                        tbResep.getValueAt(tbResep.getSelectedRow(), 2).toString().trim().substring(0,
                                Math.min(17,
                                        tbResep.getValueAt(tbResep.getSelectedRow(), 2).toString().trim().length())),
                        kodeberkas, "pages/upload/" + FileName + ".pdf"
