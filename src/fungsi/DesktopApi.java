...
public class DesktopApi {
    ...
    public static boolean print(File file) {
        if (printDESKTOP(file)) return true;
        
        return false;
    }

    private static boolean printDESKTOP(File file) {
        logOut("Trying to use Desktop.getDesktop().print() with " + file);
        try {
            if (!Desktop.isDesktopSupported()) {
                logErr("Platform is not supported.");
                return false;
            ...
            logErr("Error using desktop print.", t);
            return false;
        }
    }
}
