                            ...
                            case "statusantrean":
                                ...
                                if((!empty($header['x-token'])) && (USERNAME==$header['x-username']) && (cektoken($header['x-token'])=='true')){
                                    if(empty($decode['kodepoli'])) { 
                                        ...
                                        $nmdokter   = getOne2("SELECT maping_dokter_dpjpvclaim.nm_dokter_bpjs FROM maping_dokter_dpjpvclaim WHERE maping_dokter_dpjpvclaim.kd_dokter_bpjs='".validTeks4($decode['kodedokter'],20)."'");
                                        $hari       = strtoupper(hariindo(validTeks4($decode['tanggalperiksa'],20)));
                                        $jadwalkosong = getOne2("SELECT count(jadwal.kd_dokter) FROM jadwal WHERE jadwal.hari_kerja='$hari' AND jadwal.kd_dokter='$kddokter' AND jadwal.kd_poli='$kdpoli'");
                                        //single poli
                                        ...
                                        if(empty($kdpoli)) { 
                                            ...
                                            $response = array(
                                                'metadata' => array(
                                                    'message' =>  'Jadwal dokter '.$nmdokter.' tersebut belum tersedia, silahkan Reschedule Tanggal dan Jam praktek lainnya',
                                                    'code' => 201
                                                )
                                            );
                                            http_response_code(201);
                                          ...
                                          /*  $kuota      = getOne2("select jadwal.kuota from jadwal where jadwal.hari_kerja='$hari' and jadwal.kd_dokter='$kddokter' and jadwal.kd_poli='$kdpoli' and jadwal.jam_mulai='$jammulai:00' and jadwal.jam_selesai='$jamselesai:00'");
                                            ...
                                            $lewatjam   = fetch_array(bukaquery2("SELECT SUBSTR(jam_selesai,1,5) as jam_selesai,poliklinik.nm_poli FROM jadwal INNER JOIN poliklinik ON jadwal.kd_poli=poliklinik.kd_poli WHERE kd_dokter='$kddokter' AND jadwal.kd_poli='$kdpoli' and hari_kerja='$hari'"));
                                           if(empty($jadwal['kuota'])||$jamselesai>$lewatjam['jam_selesai']) {
                                              $response = array(
                                                  'metadata' => array(
                                                      'message' => 'Pendaftaran Ke Poli '.$lewatjam['nm_poli'].' Sudah Tutup Jam '.$lewatjam['jam_selesai'],
                                                      'code' => 201
                                                  )
                                              );
                                              http_response_code(201);    
